    ...
    <div class="relative z-10 pt-10">
        <div class="max-w-6xl mx-auto px-6 py-10 lg:py-16">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                ...
                <div class="rounded-3xl border border-white/80 bg-white/80 p-8 shadow-xl shadow-indigo-900/10 backdrop-blur lg:p-10">
                    ...
                    <?php if(!empty($success)): ?>
                        <div role="alert" class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
                    ...
                    <form method="POST" action="?route=help" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <!-- Cambiado a type="tel" por semántica -->
                                <input id="phone" name="phone" type="tel" pattern="[0-9\-\+\s\(\)]*" value="<?= htmlspecialchars($form['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm placeholder-gray-400 transition-colors focus:border-transparent focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#010b50]/30" placeholder="0412-1234567" />
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea id="message" name="message" rows="5" required maxlength="1000" class="w-full rounded-2xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm placeholder-gray-400 transition-colors focus:border-transparent focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#010b50]/30 resize-y" placeholder="Describe el detalle de tu solicitud."><?= htmlspecialchars($form['message'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                        </div>

                        <button type="submit" class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#010b50] px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition hover:bg-[#0b1f7a] focus:outline-none focus:ring-2 focus:ring-[#010b50]/40 active:scale-[0.98]">
                            Enviar solicitud
                ...
                <div class="space-y-6">
                    <div class="rounded-3xl bg-gradient-to-br from-indigo-700 via-sky-600 to-pink-500 p-8 text-white shadow-xl shadow-indigo-900/20">
                        ...
                        <div class="mt-6 space-y-4">
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 rounded-xl bg-white/10 flex items-center justify-center backdrop-blur-sm">
                    ...
                    <div class="rounded-3xl border border-white/80 bg-white/80 p-6 shadow-lg shadow-indigo-900/5 backdrop-blur">
                        <h3 class="text-lg font-semibold text-slate-900">¿Qué ocurre después?</h3>
                        <ul class="mt-3 space-y-2 text-sm text-slate-700">
                            <li class="flex items-start gap-2">
                                <span class="mt-1 h-2 w-2 rounded-full bg-emerald-500 flex-shrink-0" aria-hidden="true"></span>
