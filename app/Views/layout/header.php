...
<head>
    ...
    <style>
        ...
        .login-nav.nav-top { background: linear-gradient(125deg, rgba(1, 11, 80, 0.94), rgba(30, 58, 138, 0.88), rgba(49, 46, 129, 0.84)); border-bottom: 1px solid rgba(147, 197, 253, 0.35); backdrop-filter: blur(12px); }
        .login-nav.nav-top::after { content: ''; position: absolute; inset: auto 0 0 0; height: 2px; background: linear-gradient(90deg, rgba(147, 197, 253, 0.15), rgba(147, 197, 253, 0.9), rgba(147, 197, 253, 0.15)); background-size: 200% 100%; animation: gradient 7s ease infinite; }
        .login-nav.nav-scrolled { background-color: rgba(15, 23, 42, 0.84); border: 1px solid rgba(147, 197, 253, 0.3); box-shadow: 0 10px 25px -5px rgba(1, 11, 80, 0.45); }
    ...
    <nav id="main-nav" class="fixed z-50 nav-top <?= $showCompanyTicker ? 'with-ticker' : '' ?> <?= $isLoginRoute ? 'login-nav' : '' ?>">
        <div class="container mx-auto px-6 flex flex-col">
            <div class="flex items-center justify-between h-full">
                ...
                <?php endif; ?>
                <svg class="h-6 w-6 <?= $isLoginRoute ? 'text-blue-200 hover:text-white' : 'text-[#4F46E5] hover:text-blue-800' ?> transition-colors" 
                ...
                <span class="text-xl font-italic <?= $isLoginRoute ? 'text-blue-50' : 'text-gray-900' ?> tracking-tight">BDT<span class="<?= $isLoginRoute ? 'text-blue-300' : 'text-[#4F46E5]' ?>">.sistema</span></span>
                </div>
                <div class="flex items-center gap-3">
                    <?php if(!isset($_SESSION['user_id'])): ?>
                        <a href="?route=help" class="text-sm font-semibold <?= $isLoginRoute ? 'text-blue-100 hover:text-white' : 'text-gray-500 hover:text-[#010b50]' ?>">Centro de Ayuda</a>
                        <?php elseif($currentRoute !== 'home' && $currentRoute !== 'login'): ?>
                            ...
                            <?php if(isset($_SESSION['role']) && ($_SESSION['role'] === 'Gerente' || $_SESSION['role'] === 'Soporte')): ?>
                                <a href="?route=help_requests" class="inline-flex items-center gap-2 rounded-full bg-blue-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                ...
                                <a href="?route=ticket_stats" class="inline-flex items-center gap-2 rounded-full bg-yellow-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                        <path d="M3 3h18v4H3z" />
                                ...
                                <a href="?route=ticket_report" class="inline-flex items-center gap-2 rounded-full bg-orange-300 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                        <path d="M3 3h18v4H3z" />
            ...
            <?php if(isset($_SESSION['user_id']) && $currentRoute !== 'home' && $currentRoute !== 'login'): ?>
                <div id="mobile-menu" class="md:hidden hidden w-full pt-3 pb-2">
                    <div class="flex flex-col gap-2">
                        ...
                        <?php endif; ?>
                        <?php if(isset($_SESSION['role']) && ($_SESSION['role'] === 'Gerente' || $_SESSION['role'] === 'Soporte')): ?>
                            <a href="?route=help_requests" class="inline-flex items-center justify-center gap-2 rounded-full bg-blue-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2z"/>
                                    <path d="M8 9h8"/>
                                    <path d="M8 13h6"/>
                        ...
                        <a href="?route=ticket_report" class="inline-flex items-center justify-center gap-2 rounded-full bg-orange-300 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                <path d="M3 3h18v4H3z" />
                                <path d="M3 11h18v10H3z" />
                            </svg>
                        ...
                        <a href="?route=ticket_stats" class="inline-flex items-center justify-center gap-2 rounded-full bg-yellow-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-slate-400/40">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M3 3h18v4H3z" />
                                    <path d="M3 11h6v10H3z" />
                                    <path d="M13 11h8v10h-8z" />
